...
def DTD_h(pim, s0, Tc, W, Wpow_critic, Nc, beta, gamma, transP, reward, alpha_h, features, theta0=None, seed_sim=200,
...
def get_DTD_err_h(Theta_hat, pim, reward ,alpha_h, features, gamma, transP, transP_s2s=None, mu=None):
...
def DAC_h(DAC_dict, T, Tc, Tc_prime, Tr, N, Nc, alpha, alpha_h, beta, noise_std, s0_DAC=None, s0_DTD=None, omega0=None, \
        theta0=None, h0=None, seed_sim_DAC=100, seed_sim_DTD=200, is_print=False, is_save=False, \
        getRvg_err_every_numiter=None, getHvg_err_every_numiter=None, getDTD_err_every_numiter=None, save_folder="DACH_results/"):
    start_time = time.time()
    DAC_dict = DAC_dict.copy()
    set_seed(seed_sim_DAC)
    ...
    for t in range(T - 1):
        ...
        if getDTD_err_every_numiter is not None:
            if t % getDTD_err_every_numiter == 0:
                ...
                # DAC_dict['absolute_DTD_err'][t]=np.sum((DAC_dict['Theta'][:,:,t]-theta_star.reshape((1,-1)))**2)/DAC_dict['num_agents']
                # DAC_dict['relative_DTD_err'][t]=DAC_dict['absolute_DTD_err'][t]/np.sum(theta_star**2)
                DAC_dict['absolute_DTD_err'][t], DAC_dict['relative_DTD_err'][t] = get_DTD_err_h \
                    (DAC_dict['Theta'][:, :, t], pim, DAC_dict['reward'], DAC_dict['alpha_h'],DAC_dict['V_features'], DAC_dict['gamma'],
