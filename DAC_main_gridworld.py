...
hyps+=[{'alg':alg,'T':T,'Tc':Tc,'Tc_prime':Tc_prime,'Tr':Tr,'N':N_set[i],'Nc':Nc,\
       'alpha':alpha_set[i],'alpha_h':alpha_h,'beta':beta,'noise_std':noise_std,'s0_DAC':s_init,'s0_DTD':s_init,\
       'omega0':omega0,'theta0':None,'seed_sim_DAC':None,'seed_sim_DTD':None,'is_print':True,'is_save':False,\
       'getRvg_err_every_numiter':15,'getHvg_err_every_numiter':15,'getDTD_err_every_numiter':None,\
...
for hyp_index in range(len(hyps)):
    ...
    if 'relative_DTD_err' in results0[hyp_index].keys():
        ...
        if hyp["alg"]=='AC':
            ...
            N=int(np.sum(hyp['Nk']))
            print("The average relative TD error for our NAC with N="+str(hyp["N"])+": "+str(DTD_err.mean()))
        else:
